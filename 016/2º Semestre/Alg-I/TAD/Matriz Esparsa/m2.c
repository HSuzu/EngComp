...
int initMatrix(Matrix *m) {
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] initMatrix was caled with (%p)\n", m);
  #endif
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] initMatrix reached and with no errors. Return value 0\n\n");
  #endif
...
Node **getColumnBefore(Node **header, int column) {
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] getColumnBefore receives %p as argument.\n", header);
  #endif
...
Node **getLineBefore(Node **header, int line) {
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] getLineBefore receives %p as argument.\n", header);
  #endif
...
int linkNodeAfterColumn(Node *prev, int line, int column, int ID) {
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] linkNodeAfterColumn has seted the values (%p, %d, %d, %d) of the newNode.\n", prev, line, column, ID);
...
int addElemAt(Matrix *m, int line, int column) {
  ...
  } else if( linkNodeAfterLine(*aux, line, column, 0) < 0 ) { // erro ao criar um novo no no local desejado
    #ifdef DEBUGMSG
    fprintf(stderr, "[ERROR] addElemAt couldn't link a new node\n\n");
    #endif
...
int isEmptyColumn(Node *node) {
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] isEmptyColumn receives %p as an argument.\n", node);
  #endif
...
int removeElemAt(Matrix *m, int line, int column) {
  ...
  #ifdef DEBUGMSG
  fprintf(stderr, "[INFO] removeElemAt receives %p from getColumnBefore.\n", aux_column);
  #endif
